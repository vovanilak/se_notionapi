...
class Test:
    def __init__(self, answers, verif=[1] * 60):
        self.grouped_answers = [answers[i:i+10] for i in range(0, 60, 10)]
        self.grouped_verif = [verif[i:i+10] for i in range(0, 60, 10)]
        self.answers = self.do_kcal(answers)
        self.kcal_answers_with_verif = [round(a * b, 2) for a, b in zip(self.answers, verif)]
        self.verif = self.do_kcal(verif)
        self.merged = [round(a + b, 2) for a, b in zip(self.kcal_answers_with_verif, self.verif)]
    ...
    def normal(self, number, compet):
        return int(round(number / test.max_comp1[compet], 2) * 100)
    ...
    def get_result_sum(self, grouped_metas):
        lst = []
        for i in range(5):
            normal = sum(grouped_metas[i]) * test.percent_max_comp[i] * 100
            lst.append(normal)
        su = round(sum(lst) / test.max_sum)
